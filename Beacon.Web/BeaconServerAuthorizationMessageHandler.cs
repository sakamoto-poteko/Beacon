...
{
    public class AuthorizationMessageHandler2 : DelegatingHandler
    {
        ...
        /// <summary>
        /// Initializes a new instance of <see cref="AuthorizationMessageHandler2"/>.
        /// </summary>
        ...
        public AuthorizationMessageHandler2(
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            {
                throw new InvalidOperationException($"The '{nameof(AuthorizationMessageHandler2)}' is not configured. " +
                    $"Call '{nameof(AuthorizationMessageHandler2.ConfigureHandler)}' and provide a list of endpoint urls to attach the token to.");
            }
            ...
            if (_authorizedUris.Any(uri => uri.IsBaseOf(request.RequestUri)))
            {
                if (_lastToken == null || now >= _lastToken.Expires.AddMinutes(-5))
                {
                    ...
                    Console.WriteLine($"tokenresult: {System.Text.Json.JsonSerializer.Serialize(tokenResult.Status)}");

                    tokenResult.TryGetToken(out var token2);

                    if (tokenResult.TryGetToken(out var token))
                    {
                        _lastToken = token;
                        Console.WriteLine(_lastToken == null);
                        Console.WriteLine(_lastToken.Value);
        ...
        /// </param>
        /// <returns>This <see cref="AuthorizationMessageHandler2"/>.</returns>
        public AuthorizationMessageHandler2 ConfigureHandler(
    ...
    public class BeaconServerAuthorizationMessageHandler : AuthorizationMessageHandler
    {
        public BeaconServerAuthorizationMessageHandler(IAccessTokenProvider accessTokenProvider, NavigationManager navigationManager) :
